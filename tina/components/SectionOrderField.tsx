...
export default function SectionOrderField({ input, field, meta }: SectionOrderFieldProps) {
    const normalizedOrder = useMemo(() => {
        const saved = (input.value ?? []).filter(isSectionKey);
        return [...new Set([...saved, ...sectionOptions.map(({ value }) => value)])];
    }, [input.value]);
    ...
    return (
        <fieldset style={styles.fieldset}>
            <legend style={styles.legend}>
                {typeof field.label === "string" ? field.label : "Homepage Section Order"}
            </legend>
            {field.description && <p style={styles.help}>{field.description}</p>}

            <ol style={styles.list}>
                {normalizedOrder.map((section, index) => {
                    const option = sectionOptions.find(({ value }) => value === section);
                    ...
                    return (
                        <li
                            ...
                            style={{ ...styles.item, ...(isDragging ? styles.dragging : {}) }}
                        >
                            <span style={styles.handle} aria-hidden="true" title="Drag to reorder">⋮⋮</span>
